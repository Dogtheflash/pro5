          ...
          <div className="max-w-6xl mx-auto px-4 py-14">
            <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1.6fr]">
              ...
              <div>
                ...
                <form className="mt-6 max-w-xs" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <div className="mt-2 flex glass rounded-full overflow-hidden p-1">
                    <input type="email" required placeholder="you@email.com" aria-label="Email address" className="flex-1 bg-transparent px-4 outline-none text-sm" style={{ color: 'var(--s-fg)' }} />
                    <button className="px-4 py-2 rounded-full text-sm font-mono uppercase tracking-wider lift" style={{ background: 'var(--s-accent)', color: '#fff' }}>{t('join')}</button>
                  </div>
                </form>
                ...
                <div className="mt-6 flex flex-wrap items-center gap-3">
                  ...
                  <button
                    onClick={toggle}
                    className="inline-flex items-center gap-2 glass rounded-full px-4 py-2 text-sm lift"
              ...
              <nav aria-label={t('information')}>
                <h3 className="font-mono text-[11px] uppercase tracking-widest mb-4" style={{ color: 'var(--s-accent)' }}>{t('information')}</h3>
              ...
              <nav aria-label={t('termsPolicies')}>
                <h3 className="font-mono text-[11px] uppercase tracking-widest mb-4" style={{ color: 'var(--s-accent)' }}>{t('termsPolicies')}</h3>
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2.5">
                  {shownLegal.map((n) => <FootLink key={n.slug} slug={n.slug} label={n.label} />)}
                </ul>
                {!showAll && (
                  <button onClick={() => setShowAll(true)} className="mt-4 link-underline font-mono text-xs uppercase tracking-wider" style={{ color: 'var(--s-accent)' }}>
