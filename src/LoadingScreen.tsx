...
// A greeting from each of the eleven Southeast Asian countries, in its own
// script, cycled on the loading screen so the tour opens with a welcome from
// every nation on the itinerary.
const SEA_GREETINGS = [
  { word: 'Xin chào', country: 'Vietnam', flag: '🇻🇳' },
  { word: 'สวัสดี', country: 'Thailand', flag: '🇹🇭' },
  { word: 'Halo', country: 'Indonesia', flag: '🇮🇩' },
  { word: 'Apa khabar', country: 'Malaysia', flag: '🇲🇾' },
  { word: 'Mabuhay', country: 'Philippines', flag: '🇵🇭' },
  { word: 'Selamat', country: 'Singapore', flag: '🇸🇬' },
  { word: 'Salam', country: 'Brunei', flag: '🇧🇳' },
  { word: 'សួស្តី', country: 'Cambodia', flag: '🇰🇭' },
  { word: 'ສະບາຍດີ', country: 'Laos', flag: '🇱🇦' },
  { word: 'မင်္ဂလာပါ', country: 'Myanmar', flag: '🇲🇲' },
  { word: 'Olá', country: 'Timor-Leste', flag: '🇹🇱' },
]
...
export default function LoadingScreen({ onDone, montage = [] }: { onDone: () => void; montage?: string[] }) {
  ...
  // Hold the first greeting for ~2s, then rotate through the rest.
  useEffect(() => {
    const id = setInterval(() => setGreet((g) => (g + 1) % SEA_GREETINGS.length), 2000)
    return () => clearInterval(id)
  }, [])

  const pct = Math.round(progress * 100)
  const hello = SEA_GREETINGS[greet]
      ...
      <div className="relative flex h-full w-full flex-col items-center justify-center px-6">
        ...
        <div className="ios-spring-in relative">
          ...
          >
            ...
            />
            {/* The mark — a rotating greeting from every SEA country */}
            <div className="relative flex flex-col items-center justify-center px-3 text-center" aria-live="polite">
              <span key={greet} className="greet-in text-xl leading-none drop-shadow-[0_2px_8px_rgba(0,0,0,0.4)]" aria-hidden="true">
                {hello.flag}
              </span>
              <span
                key={`w-${greet}`}
                className="greet-in mt-1.5 block font-display text-2xl font-600 leading-tight text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.4)]"
                style={{ wordBreak: 'keep-all' }}
              >
                {hello.word}
              </span>
              <span key={`c-${greet}`} className="greet-in mt-1 block font-mono text-[9px] uppercase tracking-[0.2em] text-white/60">
                {hello.country}
              </span>
            </div>
          </div>
